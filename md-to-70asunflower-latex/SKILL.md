...
**⚠️ 注意事项**：
1. **移除手动编号**：Markdown 标题中若包含 `第一部分`、`1. `、`一、` 等手动编号前缀，必须去掉，因为 LaTeX 的 `\section` / `\subsection` 会自动编号。否则会渲染出"1 第一部分"、"1.1 1. " 等重复编号。
2. **目录处理**：`\tableofcontents` 会在 LaTeX 中自动生成"目录"标题，**不要**在它前面加自定义的`{\Large\bfseries ... 目录}`，否则会出现两个"目录"字样。
